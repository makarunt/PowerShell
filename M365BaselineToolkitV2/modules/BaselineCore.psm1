...
$script:WorkloadToConnectionDefault = @{
    ...
    # M365AdminCenter has no dedicated connection type of its own. Every control in
    # M365AdminCenterControls.psm1 today is audit-only with no API at all, so
    # this mapping is never actually exercised yet - but per the module's own
    # comment, any future control here that DOES need a live check most likely goes
    # through Microsoft Graph, so this maps here now rather than adding a speculative
    # separate connection path later.
    M365AdminCenter   = 'Graph'
}

# Recognized Set- function result statuses.
#
# AppliedPendingConfirmation and MechanismPossiblyDeprecated are produced only by
# the generic post-apply read-back-and-classify helper (Test-BaselineApplyOutcome,
# invoked from Invoke-BaselineControlApply/Invoke-BaselineControlRestore) - never
# returned directly by a control's own Set- function. Both mean "the Set- call
# itself reported success; the orchestrator just couldn't yet confirm the change
# landed" as distinct from Failed, which means the Set- call itself threw. See
# Test-BaselineApplyOutcome for the full rationale (propagation delay vs. a
# mechanism Microsoft is known to be silently retiring tenant-by-tenant).
$script:ResultStatus = @{
    Success                     = 'Success'
    Failed                      = 'Failed'
    ...
    AppliedPendingConfirmation  = 'Applied-PendingConfirmation'
    MechanismPossiblyDeprecated = 'MechanismPossiblyDeprecated'
}
...
function Get-BaselineControlCatalog {
    ...
    if (-not $AvailableFunctions) {
        ...
        $ourModuleNames = @('EntraIdControls', 'ExchangeOnlineControls', 'TeamsControls', 'SharePointOnlineControls', 'ConditionalAccessControls', 'M365AdminCenterControls')
    ...
    foreach ($control in $enabledControls) {
        ...
        if ($hasGet -and $hasSet) {
            ...
            $catalog.Add([pscustomobject]@{
                ...
                MechanismPossiblyDeprecated = if ($control.PSObject.Properties['mechanismPossiblyDeprecated']) { [bool]$control.mechanismPossiblyDeprecated } else { $false }
                GetCommand   = $getName
                SetCommand   = $setName
            })
        }
...
function Test-BaselineApplyOutcome {
    <#
    .SYNOPSIS
        Generic, control-agnostic post-apply read-back-and-classify helper.
    .DESCRIPTION
        Called after a control's Set- function has already reported success (i.e.
        it did not throw). Not every backend that succeeds a write reflects it
        immediately, or at all, in the next read: Microsoft Graph/SPO/Teams
        settings are documented and/or observed to lag a write by anywhere from
        seconds to (for a few specific, known settings) indefinitely, because
        Microsoft is silently retiring the underlying mechanism tenant-by-tenant.
        This function re-reads the control's live value via its own Get- function
        and classifies the outcome:
          - Read matches DesiredValue (first read, or after one short retry):
            'Success', Confirmed = $true.
          - Read still doesn't match after the retry, and the caller has NOT
            flagged the control as -MechanismPossiblyDeprecated: 'Applied-
            PendingConfirmation' - the change was sent, but not yet confirmed
            to have landed. Likely propagation delay; re-audit later.
          - Same persistent mismatch, but the caller HAS flagged the control as
            -MechanismPossiblyDeprecated: 'MechanismPossiblyDeprecated' instead -
            the toolkit cannot distinguish "still propagating" from "Microsoft
            has silently disabled this mechanism for this tenant" for a setting
            already known to be affected by that migration, so the report says
            so explicitly rather than presenting it as an ordinary failure.
        An actual thrown error from the Set- call itself is never routed through
        this function - Invoke-BaselineControlApply/-Restore only call it after
        a successful (non-throwing) Set-, and a Get- that throws during either
        read-back attempt is treated the same as "still doesn't match" (the
        toolkit already knows the write succeeded; it just can't confirm it).
    .PARAMETER GetCommand
        Name of the control's Get-<Id>State function used for the read-back.
    .PARAMETER DesiredValue
        The value the Set- call attempted to converge to (the same value passed
        as -DesiredValue to Set-, or a snapshot's recorded currentValue in Restore).
    .PARAMETER ComplianceMode
        Passed through to Test-BaselineCompliance for the read-back comparison.
    .PARAMETER MechanismPossiblyDeprecated
        When set, a persistent mismatch is classified as MechanismPossiblyDeprecated
        instead of Applied-PendingConfirmation.
    .PARAMETER RetryDelayMilliseconds
        Delay before the single retry read. Defaults to 2000ms; callers (and
        tests) can pass 0 to skip the wait entirely.
    .EXAMPLE
        Test-BaselineApplyOutcome -GetCommand 'Get-SharePointOnline-AzureADB2BIntegrationState' -DesiredValue $true -MechanismPossiblyDeprecated
    #>
    [CmdletBinding()]
    [OutputType([pscustomobject])]
    param(
        ...
        [Parameter()]
        [switch]$MechanismPossiblyDeprecated,
    ...
    $readBack = {
        try {
            $state = & (Get-Command $GetCommand -ErrorAction Stop)
            return [pscustomobject]@{ Value = $state.Value; Error = $null }
        }
        catch {
    ...
    $attempt1 = & $readBack
    if (-not $attempt1.Error) {
        if ((Test-BaselineCompliance -CurrentValue $attempt1.Value -DesiredValue $DesiredValue -ComplianceMode $ComplianceMode) -eq $true) {
            return [pscustomobject]@{ Status = $script:ResultStatus.Success; Confirmed = $true; ObservedValue = $attempt1.Value; Message = $null }
        }
    }
    ...
    $attempt2 = & $readBack
    if (-not $attempt2.Error) {
        if ((Test-BaselineCompliance -CurrentValue $attempt2.Value -DesiredValue $DesiredValue -ComplianceMode $ComplianceMode) -eq $true) {
            return [pscustomobject]@{ Status = $script:ResultStatus.Success; Confirmed = $true; ObservedValue = $attempt2.Value; Message = $null }
        }
    }

    $observedValue = if (-not $attempt2.Error) { $attempt2.Value } else { $attempt1.Value }

    if ($MechanismPossiblyDeprecated) {
        return [pscustomobject]@{
            Status        = $script:ResultStatus.MechanismPossiblyDeprecated
            Confirmed     = $false
            ObservedValue = $observedValue
            Message       = 'The change was sent successfully, but a follow-up read-back still does not show the desired value. This control is flagged MechanismPossiblyDeprecated: Microsoft is known to be phasing out or inconsistently honoring this specific mechanism tenant-by-tenant, so this is not necessarily a toolkit or configuration error - re-check manually if in doubt.'
        }
    }
    return [pscustomobject]@{
        Status        = $script:ResultStatus.AppliedPendingConfirmation
        Confirmed     = $false
        ObservedValue = $observedValue
        Message       = 'The change was sent successfully, but a follow-up read-back (with one retry) still does not show the desired value. This may be a propagation delay rather than a failure; re-audit later to confirm.'
    }
}
...
function Invoke-BaselineControlApply {
    ...
    param(
        ...
        [Parameter()]
        [int]$ApplyOutcomeRetryDelayMilliseconds = 2000
    )
    ...
    foreach ($entry in $Catalog) {
        ...
        try {
            ...
            if ($setResult.Status -eq $script:ResultStatus.Success) {
                ...
                $outcome = Test-BaselineApplyOutcome -GetCommand $entry.GetCommand -DesiredValue $entry.DesiredValue -ComplianceMode $entry.ComplianceMode `
                    -MechanismPossiblyDeprecated:$mechanismPossiblyDeprecated -RetryDelayMilliseconds $ApplyOutcomeRetryDelayMilliseconds
                if ($outcome.Status -ne $script:ResultStatus.Success) {
                    $setResult = [pscustomobject]@{
                        Id            = $setResult.Id
                        Status        = $outcome.Status
                        PreviousValue = $setResult.PreviousValue
                        AppliedValue  = $setResult.AppliedValue
...
function Invoke-BaselineControlRestore {
    ...
    foreach ($snap in $SnapshotControls) {
        ...
        try {
            ...
            if ($setResult.Status -eq $script:ResultStatus.Success) {
                $mechanismPossiblyDeprecated = $entry.PSObject.Properties['MechanismPossiblyDeprecated'] -and [bool]$entry.MechanismPossiblyDeprecated
                $outcome = Test-BaselineApplyOutcome -GetCommand $entry.GetCommand -DesiredValue $snap.currentValue -ComplianceMode $entry.ComplianceMode `
                    -MechanismPossiblyDeprecated:$mechanismPossiblyDeprecated -RetryDelayMilliseconds $ApplyOutcomeRetryDelayMilliseconds
                if ($outcome.Status -ne $script:ResultStatus.Success) {
                    $setResult = [pscustomobject]@{
                        Id            = $setResult.Id
