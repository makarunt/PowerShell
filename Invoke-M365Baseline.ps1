...
Import-Module (Join-Path $modulesDir 'M365AdminCenterControls.psm1') -Force -Global -WarningAction SilentlyContinue
...
try {
    ...
    switch ($Mode) {
        ...
        'Apply' {
            ...
            $pendingConfirmation = @($applyResults | Where-Object { $_.Status -eq 'Applied-PendingConfirmation' })
            $mechanismDeprecated = @($applyResults | Where-Object { $_.Status -eq 'MechanismPossiblyDeprecated' })

            # Every result is tallied here by its actual Status, not just the small set
            # of statuses this summary line happens to name - a result whose Status
            # isn't one of the ones explicitly counted above (e.g. Skipped-WhatIf, or
            # any future status) still shows up under "Other", instead of silently
            # vanishing from the console summary while still being in the full report.
            $namedCount = $succeeded.Count + $skippedOk.Count + $skippedManual.Count + $failed.Count + $pendingConfirmation.Count + $mechanismDeprecated.Count
            $otherCount = $applyResults.Count - $namedCount
            ...
            if ($pendingConfirmation.Count -gt 0) {
                Write-BaselineHost "`nControls applied but not yet confirmed (the change was sent successfully; a follow-up read couldn't yet confirm it landed - often a propagation delay, re-audit later):" 'Yellow'
                foreach ($p in $pendingConfirmation) { Write-BaselineHost "  $($p.Id): $($p.Message)" 'Yellow' }
            }

            if ($mechanismDeprecated.Count -gt 0) {
                Write-BaselineHost "`nControls flagged mechanismPossiblyDeprecated with an unconfirmed post-apply value (Microsoft is known to be retiring/migrating the underlying mechanism tenant-by-tenant - see README before treating this as a toolkit bug):" 'Yellow'
                foreach ($d in $mechanismDeprecated) { Write-BaselineHost "  $($d.Id): $($d.Message)" 'Yellow' }
            }
        ...
        'Restore' {
            ...
            $skippedManual = @($restoreResults | Where-Object { $_.Status -eq 'Skipped-Manual' })
            $skippedNoData = @($restoreResults | Where-Object { $_.Status -eq 'Skipped-NoData' })
            $pendingConfirmation = @($restoreResults | Where-Object { $_.Status -eq 'Applied-PendingConfirmation' })
            $mechanismDeprecated = @($restoreResults | Where-Object { $_.Status -eq 'MechanismPossiblyDeprecated' })

            # Same principle as Apply's summary above: every result is tallied by its
            # actual Status so nothing (Skipped-Manual, Skipped-NoData, the two new
            # read-back-classify statuses, or anything else) silently drops out of the
            # console summary while still appearing in the full restore report.
            $namedCount = $succeeded.Count + $skippedManual.Count + $skippedNoData.Count + $failed.Count + $pendingConfirmation.Count + $mechanismDeprecated.Count
            ...
            if ($mechanismDeprecated.Count -gt 0) {
                Write-BaselineHost "`nControls flagged mechanismPossiblyDeprecated with an unconfirmed post-restore value - see README before treating this as a toolkit bug:" 'Yellow'
                foreach ($d in $mechanismDeprecated) { Write-BaselineHost "  $($d.Id): $($d.Message)" 'Yellow' }
            }
