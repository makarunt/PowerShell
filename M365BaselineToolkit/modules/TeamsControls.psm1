...
function Get-Teams-BlockConsumerContactState {
    <#
    .SYNOPSIS
        Reads whether contact with unmanaged consumer Teams/Skype accounts is allowed.
    .EXAMPLE
...
function Set-Teams-BlockConsumerContactState {
    <#
    .SYNOPSIS
        Idempotently blocks/allows contact with unmanaged consumer Teams/Skype accounts.
    .PARAMETER DesiredValue
        Object: { allowTeamsConsumer, allowTeamsConsumerInbound } (both bool).
    .PARAMETER CurrentValue
        Optional pre-fetched current value.
    .EXAMPLE
        Set-Teams-BlockConsumerContactState -DesiredValue ([pscustomobject]@{allowTeamsConsumer=$false;allowTeamsConsumerInbound=$false})
    #>
    [CmdletBinding()]
    [OutputType([pscustomobject])]
    param(
    ...
    Set-CsTenantFederationConfiguration -AllowTeamsConsumer:([bool]$DesiredValue.allowTeamsConsumer) -AllowTeamsConsumerInbound:([bool]$DesiredValue.allowTeamsConsumerInbound) -ErrorAction Stop
    return [pscustomobject]@{ Id = 'Teams-BlockConsumerContact'; Status = 'Success'; PreviousValue = $current; AppliedValue = $DesiredValue; Message = 'Updated CsTenantFederationConfiguration consumer contact settings.' }
}
...
function Get-Teams-MeetingJoinDefaultsState {
    <#
    .SYNOPSIS
        Reads the Global meeting policy's lobby bypass and anonymous join settings.
    .EXAMPLE
...
function Set-Teams-MeetingJoinDefaultsState {
    <#
    .SYNOPSIS
        Idempotently sets the Global meeting policy's lobby bypass and anonymous
        join settings.
    .PARAMETER DesiredValue
